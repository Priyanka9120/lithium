...
const createUser = async function (abcd, xyz) {
  ...
  let data = abcd.body;//abcd means request
  let savedData = await userModel.create(data);
  console.log(abcd.newAtribute);
  xyz.send({ msg: savedData });//xyz means respose
};
...
const loginUser = async function (req, res) {
  ...
  if (!user)  //if username or pswword found
    return res.send({
      status: false,
  ...
  let token = jwt.sign(
    ...
    "functionup-Lithium-very-very-secret-key"    //secret key
  );
