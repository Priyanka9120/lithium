...
const getList= async function (req, res) {
    let allBooks= await BookModel.find()
        .select( { bookName: 1, authorName: 1, _id: 0})// SELECT keys that we 
...
const query = async function(req,res){
    ...
    let match =await BookModel.find({year:year})
     res.send({msg: match})
...
// //####################################################4th Api#############################################################
const getany=async function(req,res){
    let obj =req.body
    let anyBooks =await BookModel.find(obj)
    res.send({msg: anyBooks})
}

//#####################################################5th Api####################################################################################################
const price =async function(req,res){
    let pricing =await BookModel.find({indianPrice:{$or:["1001","2001","600"]}})
    res.send({msg: pricing})
}
...
//#########################################################6 Api######################################################
    const ranbook =async function(req,res){
        let avilable = await BookModel.find({$or:[{totalPages:{$gt:182}},{stockAvaible:true}]})
        res.send({msg:avilable})
    }
  


module.exports.getany = getany  //4th
module.exports.createBook = createBook

module.exports.ranbook =ranbook  //6th
module.exports.price =price   //5th
// module.exports.getBooksData = getBooksData
module.exports.query = query      //3rd
